...
struct BigInteger divide(struct BigInteger num1, struct BigInteger num2) {
    ...
    if (compare(num2, initialize("0")) == 0) {
        printf("Error: Division by zero\n");
        return quotient;
    }

    // Handle signs
    char num1Sign = num1.sign;
    char num2Sign = num2.sign;
    ...
    struct BigInteger remainder = initialize("0");

    struct node* num1Current = num1.head;

    while (num1Current != NULL) {
        int digit = num1Current->data;
        remainder = mul(remainder, initialize("10"));
        remainder = add(remainder, initialize(digit + '0'));

        struct BigInteger temp = initialize("0");
        int qDigit = 0;

        while (compare(remainder, num2) >= 0) {
            temp = add(temp, initialize("1"));
            remainder = sub(remainder, num2);
        }

        qDigit = length(temp.head) > 0 ? temp.head->data : 0;
        insert_tail(&quotient, qDigit);
    ...
    while (quotient.head != NULL && quotient.head->data == 0) {
